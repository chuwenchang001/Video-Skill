# Video-Skill · 广告视频生成(单个 Skill,供 Dify 导入)

**一个端到端技能**:输入一个商品(名称、卖点、可选产品图、投放平台、视频类型、语种、时长),输出一支可直接投放的广告短视频。技能内部按顺序走 **4 个阶段** —— 它们是本技能的**内部环节,不是四个独立技能**。

> 技能定义在根目录 **[`SKILL.md`](SKILL.md)**;本 README 是仓库导览。仓库是**说明文档 + 可复用资产(Prompt / Schema / KB)**,不是可执行代码,内容与已上线的 Video Pipeline 一致。
...
顺序执行,失败即停,已成功阶段可断点续跑。

| 阶段 | 名称 | 类型 | 作用 |
|---|---|---|---|
| ① | `PRODUCT_UNDERSTANDING` | LLM | 自动归类 + 商品理解 + 平台/视频类型策略 + 用五维量表决策唯一最优创意方向 |
| ② | `CREATIVE` | LLM | 严格承接最优方向,决策总时长并拆成 3–8 个分镜,产出可喂给视频模型的分镜脚本 |
| ③ | `VIDEO_GENERATION` | 工具 | 调火山方舟 Seedance 异步生成(15s 单段 / 30s 两段续写)+ FFmpeg 拼接 + 落 TOS |
| ④ | `SUBTITLE_BURN` | 工具 | 调外部 HTTP 服务给成片烧字幕,轮询到终态后转存 TOS,产出最终成片 |

> 每阶段的角色、Prompt、输入输出、规则详见 [`SKILL.md`](SKILL.md) 对应章节。
...
```
Video-Skill/
├── SKILL.md                  # ★ 技能定义:一个端到端技能 + 4 个内部阶段
├── DIFY_GUIDE.md             # 在 Dify 里逐节点搭建的操作手册
├── README.md                 # 本文件:仓库导览 + 节点映射速览 + 共享 env
├── .env.example              # 环境变量占位(真实 .env 不进仓库)
├── schema/                   # 各阶段输入/输出 JSON Schema(可直接贴进 Dify 结构化输出)
...
> 📘 **技能定义见 [SKILL.md](SKILL.md);完整的逐节点搭建操作手册见 [DIFY_GUIDE.md](DIFY_GUIDE.md)**(前置准备 / Start 表单 / 4 阶段节点配置 / KB 注入 / 冒烟测试 / 常见坑)。下面是速览。

## 在 Dify 里怎么搭(节点映射)

本技能在 Dify 里对应**一个 Workflow**,4 个内部阶段依次对应下列节点:

| 阶段 | Dify 节点类型 | 落地要点 |
|---|---|---|
| **① / ②**(理解决策、分镜脚本) | **LLM 节点** + 结构化输出 | `prompts/*.system.txt` 填 System、`*.user.txt` 填 User;用 Dify「结构化输出 / JSON Schema」绑定 `schema/*.output.json`;`response_format=json_object`;temperature 0.4 / 0.6 |
| **① 的知识库**(平台 / 类目 / 人群 / Few-shot) | **内联进 Prompt** 或 **Code 节点常量** | `kb/*.json` 是**配置型**知识、不是检索型,渲染进 User Prompt 即可,**不建议**做成 Dify 知识库(RAG) |
| **③ 生成 / ④ 烧字幕** | **HTTP 请求节点 / 自定义工具** | 外部接口契约见 [`SKILL.md`](SKILL.md):Seedance 异步 `create + poll`、FFmpeg `stitch`、字幕 `burn`;鉴权走对应 env |
...
| **变量传递** | 上游节点输出 → 下游 `{{}}` 引用 | 按「数据流」把 `recommendedDirection`、`scenes`、成片 URL 逐级引用 |

导入顺序建议:先把 ①② 两个 LLM 节点跑通(纯 Prompt + 结构化输出,最简单),再接 ③④ 的 HTTP + 轮询。
...
> 视频生成(Seedance)复用 `OPENAI_API_KEY` 与 `OPENAI_BASE_URL`。BYOM 账号需用自带 Seedance Model ID + Key,详见 [`SKILL.md`](SKILL.md) 阶段 ③。
